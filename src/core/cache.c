...
static char *
get_cache_path(const char *provider, const char *key)
{
	char filename[512];
	char *path;
...
char *
ani_cache_get(const char *provider, const char *key, time_t max_age)
{
	char *path;
	struct stat st;
	FILE *f;
...
bool
ani_cache_set(const char *provider, const char *key, const char *data)
{
	char *path;
	FILE *f;
	size_t data_len;
