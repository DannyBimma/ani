...
bool
ani_anilist_get_next_episode(const char *mal_id, ani_series *series)
{
	...
	resp = ani_http_post(ANILIST_GRAPHQL_URL, query_body,
	                     "application/json", NULL);
	if (resp == NULL || resp->status_code != 200) {
		LOG_WARN("AniList query failed: HTTP %ld",
			...
			if (media != NULL) {
				...
				if (next_airing != NULL) {
					...
					if (val != NULL) {
						...
						ani_parse_unix_timestamp(airing_at,
						                         &series->release.next_date);
					}
