...
void
ani_http_cleanup(void)
{
	curl_global_cleanup();
}

ani_http_config
ani_http_default_config(void)
{
...
ani_http_request_internal(const char *url, const char *method __attribute__((unused)),
                          ...
                          const ani_http_config *config)
{
	...
	for (retry = 0; retry <= config->max_retries; retry++) {
		if (retry > 0) {
			LOG_DEBUG("Retrying request (attempt %d/%d): %s",
			          retry + 1, config->max_retries + 1, url);
...
ani_http_response *
ani_http_get(const char *url, const ani_http_config *config)
{
	return ani_http_request_internal(url, "GET", NULL, NULL, config);
}

ani_http_response *
ani_http_post(const char *url, const char *body,
              const char *content_type, const ani_http_config *config)
{
	return ani_http_request_internal(url, "POST", body, content_type, config);
}

void
ani_http_response_free(ani_http_response *resp)
{
	if (resp == NULL) {
		return;
	}
